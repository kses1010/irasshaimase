  ...
  return function newWebpackFunction(
    ...
    if (isServer) {
      ...
      if (userSentryOptions.autoInstrumentServerFunctions !== false) {
        var pagesDir = _optionalChain([newConfig, 'access', _3 => _3.resolve, 'optionalAccess', _4 => _4.alias, 'optionalAccess', _5 => _5['private-next-pages']]) ;

        // Default page extensions per https://github.com/vercel/next.js/blob/f1dbc9260d48c7995f6c52f8fbcc65f08e627992/packages/next/server/config-shared.ts#L161
        var pageExtensions = userNextConfig.pageExtensions || ['tsx', 'ts', 'jsx', 'js'];
        var pageExtensionRegex = pageExtensions.map(utils.escapeStringForRegex).join('|');

        newConfig.module.rules.push({
          // Nextjs allows the `pages` folder to optionally live inside a `src` folder
          test: new RegExp(`${utils.escapeStringForRegex(projectDir)}(/src)?/pages/.*\\.(${pageExtensionRegex})`),
          use: [
            {
    ...
    // fairly massively.
    if (!isServer && _optionalChain([userSentryOptions, 'optionalAccess', _6 => _6.transpileClientSDK])) {
      // Find all loaders which apply transpilation to user code
      var transpilationRules = findTranspilationRules(_optionalChain([newConfig, 'access', _7 => _7.module, 'optionalAccess', _8 => _8.rules]), projectDir);
...
function isMatchingRule(rule, projectDir) {
  ...
  var useEntries = utils.arrayify(rule.use);
  ...
  if (!useEntries.some(entry => _optionalChain([entry, 'optionalAccess', _9 => _9.loader]) && new RegExp('next.*(babel|swc).*loader').test(entry.loader))) {
    return false;
  }
