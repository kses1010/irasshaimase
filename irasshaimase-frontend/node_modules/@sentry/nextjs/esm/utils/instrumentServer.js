...
import { configureScope, captureException, deepReadDirSync, getCurrentHub, startTransaction } from '@sentry/node';
import { hasTracingEnabled, extractTraceparentData, getActiveTransaction } from '@sentry/tracing';
import { fill, addExceptionMechanism, isString, logger, baggageHeaderToDynamicSamplingContext, stripUrlQueryAndFragment } from '@sentry/utils';
    ...
    local.run(() => {
      ...
      if (currentScope) {
        ...
        if (hasTracingEnabled() && shouldTraceRequest(nextReq.url, publicDirFiles)) {
          ...
          var baggageHeader = nextReq.headers && nextReq.headers.baggage;
          var dynamicSamplingContext = baggageHeaderToDynamicSamplingContext(baggageHeader);
...
function makeWrappedMethodForGettingParameterizedPath(
  ...
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  var wrappedMethod = async function (

    parameterizedPath

, // `findPageComponents` from nextjs 12.2.6 onward
    ...args
  ) {
    var transaction = getActiveTransaction();
    ...
    if (transaction && transaction.metadata.requestPath) {
      ...
      var newPath = typeof parameterizedPath === 'string' ? parameterizedPath : parameterizedPath.pathname;
      if (newPath) {
        var newName = transaction.name.replace(origPath, newPath);
