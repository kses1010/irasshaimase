import { captureException, startTransaction, getCurrentHub } from '@sentry/core';
import { getActiveTransaction } from '@sentry/tracing';
import { extractTraceparentData, baggageHeaderToDynamicSamplingContext } from '@sentry/utils';
import * as domain from 'domain';
import { autoEndTransactionOnResponseEnd } from './utils/responseEnd.js';
...
,
  ...
  return domain.create().bind(async () => {
    ...
    if (requestTransaction === undefined) {
      ...
      var rawBaggageString = req.headers && req.headers.baggage;
      var traceparentData =
        typeof sentryTraceHeader === 'string' ? extractTraceparentData(sentryTraceHeader) : undefined;

      var dynamicSamplingContext = baggageHeaderToDynamicSamplingContext(rawBaggageString);

      var newTransaction = startTransaction(
        {
          op: 'http.server',
    ...
    try {
      ...
      var transaction = dataFetcherSpan.transaction;
      if (transaction) {
...
export { callDataFetcherTraced, callTracedServerSideDataFetcher, getTransactionFromRequest, withErrorInstrumentation };
