...
import { rollupize } from './rollup.js';
...
async function proxyLoader( userCode) {
  ...
  var templateFile = parameterizedRoute.startsWith('/api')
    ? 'apiProxyLoaderTemplate.js'
    : 'pageProxyLoaderTemplate.js';
  var templatePath = path.resolve(__dirname, `../templates/${templateFile}`);
  ...
  var tempFilePath = path.resolve(path.dirname(this.resourcePath), `temp${Math.random()}.js`);
  ...
  var resourceFilename = path.basename(this.resourcePath);
